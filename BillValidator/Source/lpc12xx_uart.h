...
uint32_t UART_Send(LPC_UART_TypeDef *UARTx, uint8_t *txbuf, uint32_t buflen, uint32_t timeout);
uint32_t UART_Receive(LPC_UART_TypeDef *UARTx, uint8_t *rxbuf, uint32_t buflen, uint32_t timeout);

uint8_t UART_GetLineStatus(LPC_UART_TypeDef* UARTx);
#ifdef _UART_INT
void UART_ConfigInts(LPC_UART_TypeDef *UARTx, uint16_t UARTIntCfg, FunctionalState NewState);
